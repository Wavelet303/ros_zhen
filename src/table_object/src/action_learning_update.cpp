/**
 * \file action_learning.cpp
 * \author Zhen Zeng (zengzhen@umich.edu)
 * \brief search all available actions and record the ones for completing each qual state transitions in observed behavior
 */
...
bool check_qual(table_object::detect_touch_update::Response res, int qual_index)
{                                                           
    if(res.bottle_hand == (bool)human_demo[qual_index][0]       
        ...
        & res.new_detector == (bool)human_demo[qual_index][3] )
        return true;
    else
        return false;
}
...
{
	ros::init(argc, argv, "action_learning_update");
	ros::NodeHandle node;
    ros::Rate r(0.5);

	// setup all clients and publisher
	ros::ServiceClient detect_client = node.serviceClient<table_object::detect_touch_update>("detect_touch_update");
    table_object::detect_touch_update detect_srv;
    ...
    ***************************************/
    std::vector<int> q0; q0.push_back(0); q0.push_back(1); q0.push_back(0); q0.push_back(1);
    std::vector<int> q1; q1.push_back(1); q1.push_back(1); q1.push_back(0); q1.push_back(1);
    std::vector<int> q2; q2.push_back(1); q2.push_back(0); q2.push_back(0); q1.push_back(1);
		...
		for(int gripper_open=0; gripper_open<=1; gripper_open++)
		{
			for(int available_pose_index=0; available_pose_index < 12; available_pose_index++)
			{
				...
				while(!detect_service_succeed)
				{
					...
					{
						ROS_INFO("detection results: %d, %d, %d, %d", detect_srv.response.bottle_hand, detect_srv.response.bottle_tabletop, detect_srv.response.hand_tabletop, detect_srv.response.new_detector);
