...
float bottle_table_threshold = 0.04;
...
bool detect_touch(table_object::detect_touch::Request  &req,
         table_object::detect_touch::Response &res)
{
    if(l_finger_available & r_finger_available & bottle_available)
    {
        ...
        ROS_INFO("request: apply three event detectors d(bottle,hand), d(bottle,tabletop), d(hand,tabletop) %d", req.detect);
        ...
        ***************************************/
        ...
        ROS_INFO("sending back response: (bottle,hand), (bottle,tabletop), (hand,tabletop): [%d, %d, %d]", 
                 res.bottle_hand, res.bottle_tabletop, res.hand_tabletop);
        res.succeed = true;
    }else{
...
{
    ros::init(argc, argv, "touch_detector_server");
    ros::NodeHandle node;
    ros::AsyncSpinner spinner(4);
    spinner.start();
        
    ros::ServiceServer service = node.advertiseService("detect_touch", detect_touch);
    ROS_INFO("Ready to apply touch detectors.");
