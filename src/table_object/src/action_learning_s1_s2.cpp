/**
 * \file action_learning_s1_s2.cpp
 * \author Zhen Zeng (zengzhen@umich.edu)
 * \brief search all available actions and record the ones for completing the second qual state transitions in observed behavior
 */
...
//    ros::Publisher attached_object_publisher = node_handle.advertise<moveit_msgs::AttachedCollisionObject>("attached_collision_object", 1);
    ...
    ***************************************/
    std::vector<int> pre_transition_good_poses_gripper_open;
	std::vector<int> pre_good_poses_index;
	pre_transition_good_poses_gripper_open.push_back(0);
	pre_transition_good_poses_gripper_open.push_back(1);
	pre_good_poses_index.push_back(4);
	pre_good_poses_index.push_back(1);
	good_poses_index.push_back(pre_good_poses_index);
	good_poses_gripper_open.push_back(pre_transition_good_poses_gripper_open);
    ...
    ***************************************/
    ...
    for(qual_index=1; qual_index<human_demo.size(); qual_index++)
    {
			...
			for(int available_pose_index=0; available_pose_index < 7; available_pose_index++)
			{
				...
				if(qual_index == 1) // hand and bottle should be touching
				{
					...
					srand ( time(NULL) ); //initialize the random seed
		  			int RandIndex = rand() % good_poses_index[qual_index-1].size(); //generates a random number between 0 and 3
					...
					move_srv.request.target_pose=explorer.getAvailablePose(good_poses_index[qual_index-1][RandIndex]);
    ...
    ***************************************/
    ...
    for(int i=0; i<good_poses_index.size(); i++)
    {
    	printf("	transition q[%d]->q[%d]: good poses are", i, i+1);
    	for(int j=0; j<good_poses_index[i].size(); j++)
    	{
    		printf("		available_pose[%d]: %f, %f, %f, gripper open %d\n", good_poses_index[i][j], good_poses_gripper_open[i][j]);
		}
